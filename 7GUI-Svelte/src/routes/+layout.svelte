...
<div class="container" class:grid={isSidebarOpen}>
	<button onclick={() => (isSidebarOpen = !isSidebarOpen)} class="button-menu">
		{@render arrow()}
	</button>

	{#if isSidebarOpen}
		<aside transition:fly={{ x: '-400px' }} class="aside-class">
			<div class="nav-container space-y">
				<div class="title">
					{@render svelte()}
					<h2 class="text-white">7GUIs</h2>
				</div>

				<nav class="grid-gap">
					{#each navItems as { label, path }, i}
						{@const currentUrl = page.params.example === path ? 'page' : undefined}
						<a href="/examples/{path}" aria-current={currentUrl}>
							{i + 1}. {label}
						</a>
					{/each}
				</nav>
